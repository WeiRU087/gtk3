...
static guint32 next_serial = 1; // replacement of server->next_serial
...
gboolean
_gdk_broadway_server_lookahead_event (
				      const char         *types)
{
...
guint32
_gdk_broadway_server_get_last_seen_time ()
{
  return 0;
}
...
peek_all_reply() {
  BroadwayReply reply;
  Gdk_Broadway_Peek_Reply(&reply);

  all_reply = g_list_append (all_reply, &reply);
}

static BroadwayReply *
find_by_serial (guint32 serial) {
  GList *l;

  for (l = all_reply; l != NULL; l = l->next) {
    BroadwayReply *reply = l->data;

    if (reply->base.in_reply_to == serial) return reply;
  }
...
static BroadwayReply *
find_reply(guint32 serial) {
  BroadwayReply *reply;

  while(TRUE) {
    reply = find_by_serial(serial);
    
    if (reply) {
      all_reply = g_list_remove(all_reply, reply);
      break;
    }
...
_gdk_broadway_server_flush ()
{
  ...
  Gtk_Broadway_Send_Flush_Sync_QueryMouse(msg, BROADWAY_REQUEST_FLUSH);
...
_gdk_broadway_server_sync ()
{
 ...
 Gtk_Broadway_Send_Flush_Sync_QueryMouse(msg, BROADWAY_REQUEST_SYNC);
				  ...
				  guint32            *mask)
{
  ...
  Gtk_Broadway_Send_Flush_Sync_QueryMouse(msg, BROADWAY_REQUEST_QUERY_MOUSE);

  reply = find_reply (msg.serial);
...
guint32
_gdk_broadway_server_new_window (
         int x,
				 int y,
				 int width,
				 int height,
				 gboolean is_temp)
{
  ...
  msg.height = height;
  msg.is_temp = is_temp;
  msg.base.serial = next_serial++;

  Gtk_Broadway_Send_New_Window(msg, BROADWAY_REQUEST_NEW_WINDOW);

  reply = find_reply (msg.base.serial);
...
void
_gdk_broadway_server_destroy_window (gint id)
{
  BroadwayRequestDestroyWindow msg;

  msg.id = id;
 Gtk_Broadway_Send_Destroy_Show_Hide_Focus_Window(msg, BROADWAY_REQUEST_DESTROY_WINDOW);
}

gboolean
_gdk_broadway_server_window_show (gint id)
{
  BroadwayRequestShowWindow msg;

  msg.id = id;
 Gtk_Broadway_Send_Destroy_Show_Hide_Focus_Window(msg, BROADWAY_REQUEST_SHOW_WINDOW);
  
  return TRUE;
}

gboolean
_gdk_broadway_server_window_hide (gint id)
{
  BroadwayRequestHideWindow msg;

  msg.id = id;
 Gtk_Broadway_Send_Destroy_Show_Hide_Focus_Window(msg, BROADWAY_REQUEST_HIDE_WINDOW);
  
  return TRUE;
}

void
_gdk_broadway_server_window_focus (gint id)
{
  BroadwayRequestFocusWindow msg;

  msg.id = id;
 Gtk_Broadway_Send_Destroy_Show_Hide_Focus_Window(msg, BROADWAY_REQUEST_FOCUS_WINDOW);
}

void
_gdk_broadway_server_window_set_transient_for (
					       gint id, gint parent)
{
  ...
  msg.id = id;
  msg.parent = parent;
 Gtk_Broadway_Send_Set_Transient_For(msg, BROADWAY_REQUEST_SET_TRANSIENT_FOR);
}
				    ...
				    cairo_surface_t *surface)
{
 ...
 Gtk_Broadway_Send_Window_Update(msg, BROADWAY_REQUEST_UPDATE);
					 ...
					 int height)
{
 ...
 Gtk_Broadway_Send_Move_Resize(msg, BROADWAY_REQUEST_MOVE_RESIZE);

  return TRUE;
}

GdkGrabStatus
_gdk_broadway_server_grab_pointer (
				   gint id,
...
guint32
_gdk_broadway_server_ungrab_pointer (
				     guint32    time_)
{
...
void
_gdk_broadway_server_set_show_keyboard (
                                        gboolean show)
{
  BroadwayRequestSetShowKeyboard msg;

  msg.show_keyboard = show;
 Gtk_Broadway_Send_Set_Show_Keyboard(msg, BROADWAY_REQUEST_SET_SHOW_KEYBOARD);
}
